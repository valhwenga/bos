...
const Sent = () => {
  const [list, setList] = useState<MailMessage[]>(EmailStore.byFolder("sent"));
  const [q, setQ] = useState("");
  const refresh = () => setList(EmailStore.byFolder("sent"));
  useEffect(()=>{ refresh(); }, []);
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold mb-1">Sent</h1>
          <p className="text-sm text-muted-foreground">Your sent emails</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-secondary/50">
              <tr>
                <th className="text-left p-3 text-sm">TO</th>
            ...
            <tbody>
              {filtered.map(m => (
                <tr key={m.id} className="border-t hover:bg-secondary/30 cursor-pointer" onClick={()=> navigate(`/email/${m.id}`)}>
                  <td className="p-3 text-sm">{m.to.map(t=> t.name||t.email).join(', ')}</td>
